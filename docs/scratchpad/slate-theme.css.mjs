// Console Slate — design-lab override layer (iteration 3: polish sweep).
...
// HARD RULE: never a left leading-edge accent stroke/spine. Status = full tint,
// top rule, full-perimeter ring, filled badge, or icon — never a left bar.

export const SLATE_OVERRIDES = `
/* ============================================================
   CONSOLE SLATE — tokens (palette + border-first elevation + grid)
   ============================================================ */
:root{
  ...
  /* accent is set inline on <html> by the runtime -> !important to win in the lab */
  --ds-accent:#3a5ccc!important; --ds-accent-2:#3350b8!important; --ds-accent-tint:rgba(58,92,204,.09)!important;
  /* elevation: tinted ring + one earned layered shadow */
  ...
  --ds-frame:1400px;   /* wider canvas (user request) */
}
[data-theme="dark"]{
  ...
  --ds-accent-tint-2:rgba(125,155,255,.22);
  --ds-accent:#7d9bff!important; --ds-accent-2:#93abff!important; --ds-accent-tint:rgba(125,155,255,.15)!important;
...
/* quiet structural grid behind the page (screen only, masked, pointer-transparent) */
@media screen{
  body::before{content:"";position:fixed;inset:0;z-index:-1;pointer-events:none;
    background-image:linear-gradient(var(--ds-grid) 1px,transparent 1px),linear-gradient(90deg,var(--ds-grid) 1px,transparent 1px);
...
/* ============================================================
   Base type — density, tracking ramp, tabular figures
   ============================================================ */
...
/* ============================================================
   Masthead — frosted, scroll-elevating
   ============================================================ */
.ds-topbar{background:color-mix(in srgb,var(--ds-bg) 78%,transparent);backdrop-filter:blur(12px) saturate(140%);-webkit-backdrop-filter:blur(12px) saturate(140%);border-bottom:1px solid transparent;transition:border-color var(--ds-t-base) var(--ds-ease),box-shadow var(--ds-t-base) var(--ds-ease)}
.ds-topbar.is-stuck{border-bottom-color:var(--ds-line);box-shadow:0 1px 0 hsl(var(--ds-shadow-color)/.05),inset 0 1px 0 hsl(0 0% 100%/.5)}
[data-theme="dark"] .ds-topbar.is-stuck{box-shadow:0 1px 0 hsl(var(--ds-shadow-color)/.5),inset 0 1px 0 hsl(225 60% 100%/.05)}
...
.ds-content>.ds-section .ds-section-titles h2::before{
  ...
  font-size:11px;font-weight:600;letter-spacing:var(--ds-tr-caps);padding:0;margin-bottom:7px;border-radius:0}
...
/* on-navigate target tint (motion-gated) */
...
/* TOC — active tint chip, no left rail */
...
/* ============================================================
   Hero — two-zone grid; sideCards -> right instrument panel
   ============================================================ */
...
[data-block="hero"]>.ds-meta{
  ...
  border:0;border-radius:13px;background:var(--ds-bg-2);box-shadow:var(--ds-elev-1);overflow:hidden;display:block;
  background-image:linear-gradient(180deg,hsl(0 0% 100%/.5),transparent 44px)}
...
/* ============================================================
   KPI stats — fused strip; summary cards — tonal + border-first elevation
   ============================================================ */
...
.ds-card{border:0;border-radius:13px;background:var(--ds-bg-2);box-shadow:var(--ds-elev-1),inset 0 1px 0 hsl(0 0% 100%/.55);transition:box-shadow 380ms var(--ds-ease),transform 380ms var(--ds-ease)}
[data-theme="dark"] .ds-card{box-shadow:var(--ds-elev-1),inset 0 1px 0 hsl(225 60% 100%/.06)}
...
@media (hover:hover){
  .ds-card:hover{transform:translateY(-1px);box-shadow:var(--ds-elev-2),inset 0 1px 0 hsl(0 0% 100%/.6);transition-duration:var(--ds-t-fast)}
}

/* ============================================================
   Callouts — tinted surface + 2px TOP rule + icon (NO left bar)
   ============================================================ */
.ds-callout{position:relative;border:0;border-top:2px solid var(--ds-line-strong);box-shadow:var(--ds-ring);background:var(--ds-bg-2);border-radius:11px;padding:14px 16px 14px 46px;color:var(--ds-ink-2);-webkit-print-color-adjust:exact;print-color-adjust:exact}
...
.ds-callout.tone-tip{border-top-color:var(--ds-accent);background:color-mix(in oklab,var(--ds-accent) 7%,var(--ds-bg-2))}
.ds-callout.tone-tip::before{content:"\\2726";background:var(--ds-accent)}
.ds-callout.tone-ok{border-top-color:var(--ds-ok);background:var(--ds-ok-tint)}
.ds-callout.tone-ok::before{content:"\\2713";background:var(--ds-ok)}
.ds-callout.tone-warn{border-top-color:var(--ds-warn);background:var(--ds-warn-tint)}
.ds-callout.tone-warn::before{content:"!";background:var(--ds-warn)}
.ds-callout.tone-danger{border-top-color:var(--ds-danger);background:var(--ds-danger-tint)}
.ds-callout.tone-danger::before{content:"!";background:var(--ds-danger)}
.ds-callout.tone-info{border-top-color:var(--ds-info);background:var(--ds-info-tint)}
.ds-callout.tone-info::before{content:"i";background:var(--ds-info)}
[data-theme="dark"] .ds-callout::before{color:#0c0e13}

/* ============================================================
   Tables — framed wrap, mono header, theme-relative zebra, accent hover
   ============================================================ */
...
/* Decision matrix winner — full-row tint + bold (badge via .ds-pill), no bar */
...
/* Risk register — severity heat pills (no bar) */
...
/* Status chips — full semantic map */
...
/* ============================================================
   Process / review rows — full-perimeter semantic ring (no spine)
   ============================================================ */
.ds-ritem{border-radius:11px}
.ds-pitem.verdict-approve{box-shadow:0 0 0 1px color-mix(in oklab,var(--ds-ok) 40%,transparent),var(--ds-elev-1)}
.ds-pitem.verdict-block{box-shadow:0 0 0 1px color-mix(in oklab,var(--ds-danger) 40%,transparent),var(--ds-elev-1)}
.ds-pitem.verdict-revise,.ds-pitem.verdict-retry{box-shadow:0 0 0 1px color-mix(in oklab,var(--ds-warn) 40%,transparent),var(--ds-elev-1)}
.ds-pitem.verdict-approve,.ds-pitem.verdict-block,.ds-pitem.verdict-revise,.ds-pitem.verdict-retry{border-color:transparent}
.ds-ritem.selected{box-shadow:0 0 0 1px var(--ds-accent),var(--ds-elev-1);border-color:transparent}

/* Trust claim — tint + 2px TOP rule + ring (no left bar) */
.ds-trust-claim{border:0;border-left:0;box-shadow:var(--ds-ring);border-top:2px solid var(--ds-line-strong);border-radius:12px;padding:15px 17px;background:var(--ds-bg-2);-webkit-print-color-adjust:exact;print-color-adjust:exact}
.ds-trust-claim.status-verified,.ds-trust-claim.status-accepted{border-top-color:var(--ds-ok);background:var(--ds-ok-tint)}
.ds-trust-claim.status-partial,.ds-trust-claim.status-unverified{border-top-color:var(--ds-warn);background:var(--ds-warn-tint)}
.ds-trust-claim.status-disputed,.ds-trust-claim.status-rejected{border-top-color:var(--ds-danger);background:var(--ds-danger-tint)}

/* detail / key-value grids — tighter mono labels */
...
/* code language accents inherit indigo */
.ds-code-bar .ds-lang,.ds-codeedit-bar .ds-lang{color:var(--ds-accent)}

/* ============================================================
   ITERATION 2 — layout & padding fixes (retained)
   ============================================================ */
...
/* release gate: title left, status right, notes textarea full width */
...
/* ============================================================
   Interactive polish — focus halo, selection, controls non-selectable
   ============================================================ */
...
// Lab-only: scroll-elevate the topbar. (In the product this hooks into runtime.mjs.)
export const SLATE_SCRIPT = `
