...
(function(){
  ...
  function reviewCount(){var n=$$("[data-select]:checked").length;var c=$("[data-review-count]");if(c)c.textContent=n+" selected";}
  ...
  var rsearch=$("[data-review-search]"),ronly=$("[data-review-only]"),rexpand=$("[data-review-expand]");
  function reviewRoot(){return (rsearch&&rsearch.closest(".ds-reviewboard"))||(ronly&&ronly.closest(".ds-reviewboard"))||(rexpand&&rexpand.closest(".ds-reviewboard"))||document;}
  function reviewFilter(){var q=((rsearch&&rsearch.value)||"").trim().toLowerCase();var only=ronly&&ronly.checked;$$(".ds-ritem",reviewRoot()).forEach(function(i){var txt=i.getAttribute("data-text")||"";var cb=i.querySelector("[data-select]");var sel=cb&&cb.checked;i.style.display=((!q||txt.indexOf(q)>=0)&&(!only||sel))?"":"none";});}
  if(rsearch)rsearch.addEventListener("input",reviewFilter);
  if(ronly)ronly.addEventListener("change",reviewFilter);
  if(rexpand)rexpand.addEventListener("click",function(){var items=$$(".ds-ritem",reviewRoot());var anyClosed=items.some(function(i){return !i.classList.contains("open");});items.forEach(function(i){i.classList.toggle("open",anyClosed);});rexpand.textContent=anyClosed?"Collapse all":"Expand all";});
  ...
  function processCount(){var n=$$("[data-process-verdict]").filter(function(s){return s.value&&s.value!=="undecided";}).length;var c=$("[data-process-count]");if(c)c.textContent=n+" verdict"+(n===1?"":"s");}
  ...
  processCount();
  var ep=$("[data-export-process]");if(ep)ep.addEventListener("click",function(){download(slug+".process.json",JSON.stringify({schema:"dossier.process/v1",slug:slug,process:state.process},null,2),"application/json");});
  var ip=$("[data-import-process]");if(ip)ip.addEventListener("click",function(){var inp=document.createElement("input");inp.type="file";inp.accept="application/json";inp.onchange=function(){var f=inp.files[0];if(!f)return;var r=new FileReader();r.onload=function(){try{var d=JSON.parse(r.result);var proc=d.process||d.items||d;Object.keys(proc).forEach(function(id){state.process[id]=proc[id];var sel=$("[data-process-verdict='"+id+"']");if(sel&&proc[id].verdict){sel.value=proc[id].verdict;processVerdictClass(sel.closest(".ds-pitem"),sel.value);}var ta=$("[data-process-notes='"+id+"']");if(ta&&proc[id].notes!=null)ta.value=proc[id].notes;});save();processCount();processFilter();toast("Imported process state");}catch(e){toast("Import failed");}};r.readAsText(f);};inp.click();});
  ...
  var psearch=$("[data-process-search]"),ponly=$("[data-process-only]"),pexpand=$("[data-process-expand]");
  function processFilter(){var q=((psearch&&psearch.value)||"").trim().toLowerCase();var only=ponly&&ponly.checked;$$(".ds-pitem").forEach(function(i){var txt=i.getAttribute("data-text")||"";var sel=i.querySelector("[data-process-verdict]");var has=sel&&sel.value&&sel.value!=="undecided";i.style.display=((!q||txt.indexOf(q)>=0)&&(!only||has))?"":"none";});}
  if(psearch)psearch.addEventListener("input",processFilter);
  if(ponly)ponly.addEventListener("change",processFilter);
  if(pexpand)pexpand.addEventListener("click",function(){var items=$$(".ds-pitem");var anyClosed=items.some(function(i){return !i.classList.contains("open");});items.forEach(function(i){i.classList.toggle("open",anyClosed);});pexpand.textContent=anyClosed?"Collapse all":"Expand all";});

  function editorMark(id,dirty){var shell=$("[data-editor-shell='"+id+"']"),label=$("[data-editor-state='"+id+"']");if(shell)shell.classList.toggle("dirty",!!dirty);if(label)label.textContent=dirty?"dirty":"clean";}
  state.editors=state.editors||{};
  $$("[data-code-editor]").forEach(function(ta){var id=ta.getAttribute("data-code-editor");var saved=state.editors[id];ta.__original=ta.value;if(saved&&saved.text!=null)ta.value=saved.text;function sync(){var dirty=ta.value!==ta.__original;state.editors[id]={text:ta.value,lang:ta.getAttribute("data-editor-lang")||"",filename:ta.getAttribute("data-editor-filename")||"",targetPath:ta.getAttribute("data-editor-target")||"",title:ta.getAttribute("data-editor-title")||"",dirty:dirty};editorMark(id,dirty);save();}ta.addEventListener("input",sync);editorMark(id,ta.value!==ta.__original);var reset=$("[data-editor-reset='"+id+"']");if(reset)reset.addEventListener("click",function(){ta.value=ta.__original;sync();});if(typeof window.DossierEditorEnhancer==="function"){try{window.DossierEditorEnhancer({id:id,textarea:ta,language:ta.getAttribute("data-editor-lang")||"",filename:ta.getAttribute("data-editor-filename")||"",targetPath:ta.getAttribute("data-editor-target")||"",title:ta.getAttribute("data-editor-title")||""});}catch(e){}}});
  ...
  function importEditors(){var inp=document.createElement("input");inp.type="file";inp.accept="application/json";inp.onchange=function(){var f=inp.files[0];if(!f)return;var r=new FileReader();r.onload=function(){try{var d=JSON.parse(r.result);var edits=d.edits||d.items||d;Object.keys(edits).forEach(function(id){var entry=edits[id];if(!entry||entry.text==null)return;state.editors[id]=entry;var ta=$("[data-code-editor='"+id+"']");if(ta){ta.value=entry.text;editorMark(id,ta.value!==ta.__original);}});save();toast("Imported editor edits");}catch(e){toast("Import failed");}};r.readAsText(f);};inp.click();}
  ...
  function exportVerdicts(){download(slug+".verdicts.json",JSON.stringify({schema:"dossier.verdicts/v1",slug:slug,verdicts:state.verdicts},null,2),"application/json");}
  function importVerdicts(){var inp=document.createElement("input");inp.type="file";inp.accept="application/json";inp.onchange=function(){var f=inp.files[0];if(!f)return;var r=new FileReader();r.onload=function(){try{var d=JSON.parse(r.result);var verdicts=d.verdicts||d.items||d;Object.keys(verdicts).forEach(function(id){state.verdicts[id]=verdicts[id];var sel=$("[data-verdict-gate='"+id+"']");if(sel&&verdicts[id].verdict)sel.value=verdicts[id].verdict;var ta=$("[data-verdict-notes='"+id+"']");if(ta&&verdicts[id].notes!=null)ta.value=verdicts[id].notes;});save();toast("Imported verdicts");}catch(e){toast("Import failed");}};r.readAsText(f);};inp.click();}
  ...
  $$("[data-release-gate]").forEach(function(cb){var id=cb.getAttribute("data-release-gate");if(state.release[id]&&state.release[id].done!=null)cb.checked=!!state.release[id].done;function sync(){state.release[id]=state.release[id]||{};state.release[id].done=cb.checked;save();}cb.addEventListener("change",sync);});
  ...
  function exportRelease(){download(slug+".release.json",JSON.stringify({schema:"dossier.release/v1",slug:slug,release:state.release},null,2),"application/json");}
  function importRelease(){var inp=document.createElement("input");inp.type="file";inp.accept="application/json";inp.onchange=function(){var f=inp.files[0];if(!f)return;var r=new FileReader();r.onload=function(){try{var d=JSON.parse(r.result);var rel=d.release||d.gates||d;Object.keys(rel).forEach(function(id){state.release[id]=rel[id];var cb=$("[data-release-gate='"+id+"']");if(cb&&rel[id].done!=null)cb.checked=!!rel[id].done;var ta=$("[data-release-notes='"+id+"']");if(ta&&rel[id].notes!=null)ta.value=rel[id].notes;});save();toast("Imported release state");}catch(e){toast("Import failed");}};r.readAsText(f);};inp.click();}
