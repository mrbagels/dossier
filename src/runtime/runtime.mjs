...
(function(){
  ...
  $$("[data-code-editor]").forEach(function(ta){var id=ta.getAttribute("data-code-editor");var saved=state.editors[id];ta.__original=ta.value;if(saved&&saved.text!=null)ta.value=saved.text;function sync(){var dirty=ta.value!==ta.__original;state.editors[id]={text:ta.value,lang:ta.getAttribute("data-editor-lang")||"",filename:ta.getAttribute("data-editor-filename")||"",targetPath:ta.getAttribute("data-editor-target")||"",dirty:dirty};editorMark(id,dirty);save();}ta.addEventListener("input",sync);editorMark(id,ta.value!==ta.__original);var reset=$("[data-editor-reset='"+id+"']");if(reset)reset.addEventListener("click",function(){ta.value=ta.__original;sync();});if(typeof window.DossierEditorEnhancer==="function"){try{window.DossierEditorEnhancer({id:id,textarea:ta,language:ta.getAttribute("data-editor-lang")||"",filename:ta.getAttribute("data-editor-filename")||"",targetPath:ta.getAttribute("data-editor-target")||""});}catch(e){}}});
  function exportEditors(){var edits={};$$("[data-code-editor]").forEach(function(ta){var id=ta.getAttribute("data-code-editor");edits[id]={text:ta.value,lang:ta.getAttribute("data-editor-lang")||"",filename:ta.getAttribute("data-editor-filename")||"",targetPath:ta.getAttribute("data-editor-target")||"",dirty:ta.value!==ta.__original};});download(slug+".edits.json",JSON.stringify({schema:"dossier.edits/v1",slug:slug,edits:edits},null,2),"application/json");}
